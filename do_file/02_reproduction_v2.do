...
*  Reproduce all robustness checks
*  Author: Kirara Homma
***************************************************
...
did_multiplegt_dyn treecover cell_id year trt_overall_irrigation, effects(14)  placebo(5) controls(trt_overall_road trt_overall_else temperature precipitation) cluster(commune_id) weight(cell_count_30m) save_results("$dt_output\result_new_estimator")
...
merge 1:1 commune_id year using "D:\Shared Data\homma\PhD_2025\Replication_game\data\tmp\tmf_deforestation_v2.dta"
drop _merge
save "D:\Shared Data\homma\PhD_2025\Replication_game\data\commune_tmf_deforestation.dta", replace
*/
* analysis 
use "D:\Shared Data\homma\PhD_2025\Replication_game\data\commune_tmf_deforestation.dta", clear
...
* (1), (2)
// log
reghdfe log_ndvi trt_overall_road trt_overall_irrigation trt_overall_else temperature precipitation [aw=cell_count_30m], absorb(cell_id year) cluster(commune_id year)
estimates store out1_9

// asinh
reghdfe asinh_ndvi trt_overall_road trt_overall_irrigation trt_overall_else temperature precipitation [aw=cell_count_30m], absorb(cell_id year) cluster(commune_id year)
estimates store out1_10


* (3), (4)
// log
reghdfe log_treecover trt_overall_road trt_overall_irrigation trt_overall_else temperature precipitation [aw=cell_count_30m], absorb(cell_id year) cluster(commune_id year)
estimates store out2_9

// log(y+1)
reghdfe log_treecover_plus1 trt_overall_road trt_overall_irrigation trt_overall_else temperature precipitation [aw=cell_count_30m], absorb(cell_id year) cluster(commune_id year)
estimates store out2_10

// asinh
reghdfe asinh_treecover trt_overall_road trt_overall_irrigation trt_overall_else temperature precipitation [aw=cell_count_30m], absorb(cell_id year) cluster(commune_id year)
estimates store out2_11
...
estimates store out1_0_original
