...
*  Reproduce robustness checks in Section 3.1
*  using TWFE
*  Author: Kirara Homma
***************************************************
...
* estimation
postutil clear
postfile myresults outcome treatment origpath str50 clustering str50 out str50 treat str50 control b se p using "$dt_output\repframe_multiverse_twfe_out1.dta", replace
	...
	foreach outcome in ndvi asinh_ndvi {
		foreach treat in trt_overall_road trt_overall_road_wt3km trt_overall_road_wt4km {
			foreach control in "" ///
                ...
                else if strpos(`"`control'"', "bombing") > 0 {
                    local ctrllab "cont1"
                }
                else {
                    local ctrllab "cont2"
				...
				reghdfe `outcome' `treat' trt_overall_irrigation trt_overall_else temperature precipitation `control' ///
                ...
                if `sec' == 0 | missing(`sec') {
                    ...
                    local pv = 2 * ttail(e(df_r), abs(`bc'/`sec'))
				}
...
export delimited "$dt_output\repframe_multiverse_twfe_out1.csv", replace
save "$dt_output\repframe_multiverse_twfe_out1.dta", replace
restore
...
* estimation
postutil clear
postfile myresults outcome treatment origpath str50 clustering str50 out str50 treat str50 control b se p using "$dt_output\repframe_multiverse_twfe_out2.dta", replace
	...
	foreach outcome in ndvi asinh_ndvi {
		foreach treat in trt_overall_irrigation trt_overall_irri_wt3km trt_overall_irri_wt4km {
			foreach control in "" ///
				...
				reghdfe `outcome' trt_overall_road `treat' trt_overall_else temperature precipitation `control' ///
				[aw=cell_count_30m], absorb(cell_id year) cluster(`clustering')
...
* save as csv
preserve
use "$dt_output\repframe_multiverse_twfe_out2.dta", clear
replace origpath = 1 in 1 // the original estimation
export delimited "$dt_output\repframe_multiverse_twfe_out2.csv", replace
save "$dt_output\repframe_multiverse_twfe_out2.dta", replace
restore
...
* estimation
postutil clear
postfile myresults outcome treatment origpath str50 clustering str50 out str50 treat str50 control b se p using "$dt_output\repframe_multiverse_twfe_out3.dta", replace
...
* save as csv
preserve
use "$dt_output\repframe_multiverse_twfe_out3.dta", clear
replace origpath = 1 in 1 // the original estimation
export delimited "$dt_output\repframe_multiverse_twfe_out3.csv", replace
save "$dt_output\repframe_multiverse_twfe_out3.dta", replace
restore
...
* estimation
postutil clear
postfile myresults outcome treatment origpath str50 clustering str50 out str50 treat str50 control b se p using "$dt_output\repframe_multiverse_twfe_out4.dta", replace
...
* save as csv
preserve
use "$dt_output\repframe_multiverse_twfe_out4.dta", clear

replace origpath = 1 in 1 // the original estimation
export delimited "$dt_output\repframe_multiverse_twfe_out4.csv", replace
save "$dt_output\repframe_multiverse_twfe_out4.dta", replace
restore
